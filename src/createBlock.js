...
//   - исправить пути к генерируемым папкам и файлам, если блоки проекта лежат не в ./blocks/
...
if(blockName) {
  ...
  let dirPath = 'blocks/' + blockName + '/'; // полный путь к создаваемой папке блока
  mkdirp(dirPath, function(err){                 // создаем
    ...
    // Нет ошибки, поехали!
      ...
      extensions.forEach(function(extention){
        ...
        let pugFileImport = '';                               // будущая конструкция импорта файла pug
        ...
        // Если это pug
          ...
          pugFileImport = '// include \'./blocks/' + blockName + '/' + blockName + '.pug\'';
          fileContent = pugFileImport + '\n\n\n.' + blockName;
          fileCreateMsg = 'Для инклуда pug: ' + pugFileImport;
        }
